...
def load():
    ...
    def scheduleprogram():
        ...
        print(directory)
        print(filename)
        time = gui_input("Please enter the time for program to run each day")
        print(time)
        nametask = gui_input("Please enter name for task")
        print(nametask)
        process = subprocess.Popen(['SCHTASKS /Create /SC DAILY /TN' + nametask + '/TR' + "'"+filename+"'" + '/ST', time], stdout=subprocess.PIPE, stderr=None, shell=True)
...
#Launch the shell command:
    ...
    button15 = tk.Button(text="Schedule Program", command=scheduleprogram)              
    ...
    button15.grid(row=4, column=0, padx=10)
